...
async function processData(json) {
    ...
    async function getOrCreateCategory(categoryData) {
        ...
        let category = await db.categories
            .where('book_id').equals(props.bookId)
            .and(c => c.name === name)
            .first()
    ...
    <div class="space-y-6">
        <div class="bg-blue-50 border border-blue-100 rounded-sm p-4">
            ...
            <p class="text-sm text-blue-600 mt-1">
                Import data from the old JSON format. This will create missing contacts, categories, payment modes, and products.
            </p>
        </div>
