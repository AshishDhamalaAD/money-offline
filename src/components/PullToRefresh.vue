...
<template>
  ...
  >
    <!-- Loading Indicator -->
    <div 
      class="absolute top-0 left-0 right-0 flex justify-center items-center pointer-events-none z-0"
      :style="{ 
        height: `${threshold}px`,
    ...
    >
      <div v-if="isLoading" class="animate-spin rounded-full h-6 w-6 border-2 border-gray-400 border-t-gray-800"></div>
      <div v-else class="transform transition-transform duration-200" :style="{ transform: `rotate(${pullDistance * 2}deg)` }">
    ...
    <!-- Content -->
    <div 
      class="relative z-10 bg-gray-50 transition-transform duration-200 ease-out min-h-screen"
